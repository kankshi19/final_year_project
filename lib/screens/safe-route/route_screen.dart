...
class _RouteScreenState extends State<RouteScreen> {
  ...
  try {
    final rawRoutes = await ApiService.fetchRoutes(
    ...
    routes = await Future.wait(
      rawRoutes.map(
        (route) => EnhancedRoute.fromRouteData(
          route,
          startLat: widget.startLocation.latitude,
          startLng: widget.startLocation.longitude,
          endLat: widget.destinationLocation.latitude,
          endLng: widget.destinationLocation.longitude,
          gmapsApiKey: google_mapApiKey,
    ...
    //   // Calculate safety score using the RouteAnalyzer logic̥̥
  ...
  Widget _buildCrowdInfo(EnhancedRoute route) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Divider(),
        ...
        const SizedBox(height: 8),
        _buildSafetyIndicator(route.safetyScore),
      ],
    );
  }

  Widget _buildRouteCard(EnhancedRoute route, int index) {
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                CircleAvatar(
                  backgroundColor: _getSafetyColor(route.safetyScore.toDouble()),
                  child: Icon(
                    _getSafetyIcon(route.safetyScore.toDouble()),
                    color: Colors.white,
                  ),
                ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 4),
                      Text(
                        'Duration: ${route.durationLabel ?? 'N/A'}',
                        style: const TextStyle(color: Colors.grey),
                      ),
                      Text(
                        'Distance: ${route.distanceLabel?? 'N/A'}',
                        style: const TextStyle(color: Colors.grey),
                      ),
                      Text(
                        'Safety Score: ${route.safetyScore.toStringAsFixed(1)}%',
                        style: TextStyle(
                          color: _getSafetyColor(route.safetyScore),
                          fontWeight: FontWeight.bold,
                        ),
                      ),
                    ],
            ...
            ElevatedButton(
  onPressed: () async {
    final url = route.directionsLink;
    print(url);
  ...
  style: ElevatedButton.styleFrom(
    backgroundColor: const Color.fromARGB(255, 28, 134, 129),
